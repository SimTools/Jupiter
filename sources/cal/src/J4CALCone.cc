...
{   
   if(!GetLV()){
      ...
      J4CALParameterList                  *list     = OpenParameterList();  	
      J4CALParameterList::J4CALTowerParam *curparam = list->GetTowerParam(GetMyID()); 

      G4double height = curparam->GetHeight();
      G4double rmin   = curparam->GetR();
      G4double rmax   = rmin + height; 
      ...
      // SetVisAttribute ----//
      PaintLV(list->GetConeVisAtt(), list->GetConeColor());
