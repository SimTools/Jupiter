...
static const G4double mykCarTolerance = 1.E-9*mm;
                ...
                G4VPhysicalVolume* pPhysVol)
{
  ...
  // just forget.
  if(CurrentProposedStepLength<mykCarTolerance) return DBL_MAX;
  ...
  if( fNoZeroStep > fActionThreshold_NoZeroSteps )
  {
    ...
    if(   (fNoZeroStep < fSevereActionThreshold_NoZeroSteps)
       && (stepTrial > 1000.0*mykCarTolerance) )
    {
    ...
    else
    {
      ...
      //
      if( stepTrial > 1000.0*mykCarTolerance )
	decreaseFactor = 0.25;     // Try slow decreases
      else if( stepTrial > 100.0*mykCarTolerance )
	decreaseFactor= 0.5;       // Try slower decreases
      else if( stepTrial > 10.0*mykCarTolerance )
...
#endif
     if( stepTrial == 0.0 )
       //     if( stepTrial < mykCarTolerance*0.1 )
     {
  ...
  { 
    ...
    do_loop_count++;
        ...
        && (StepTaken + mykCarTolerance < CurrentProposedStepLength)  
