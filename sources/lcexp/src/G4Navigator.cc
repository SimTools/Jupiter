                                         ...
                                         G4double &pNewSafety)
{
  ...
  static G4double fAccuracyForWarning   = mykCarTolerance,
                  fAccuracyForException = 1000*mykCarTolerance;
#endif
  ...
  if( newLocalPoint != fLastLocatedPointLocal )
  {
    ...
    if ( moveLenSq >= mykCarTolerance*mykCarTolerance )
    {
#ifdef G4VERBOSE
  ...
  fLocatedOnEdge   = fLastStepWasZero && (Step<0.5*mykCarTolerance);
  fLastStepWasZero = (Step<0.5*mykCarTolerance);
